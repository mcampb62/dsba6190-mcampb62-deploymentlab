...
resource "azurerm_application_insights" "insights" {
  name                = "${var.class_name}${var.student_name}${var.environment}${random_integer.deployment_id_suffix.result}ins"
  location            = azurerm_resource_group.rg.location
  resource_group_name = azurerm_resource_group.rg.name
...
resource "azurerm_storage_account" "mlstore" {
  name                     = "${var.class_name}${var.student_name}${var.environment}${random_integer.deployment_id_suffix.result}sa"
  location                 = azurerm_resource_group.rg.location
  resource_group_name      = azurerm_resource_group.rg.name
  account_tier             = "Standard"
...
resource "azurerm_machine_learning_workspace" "ml" {
  ...
  storage_account_id      = azurerm_storage_account.mlstore.id
...
resource "azurerm_maps_account" "map" {
  name                = "${var.class_name}${var.student_name}${var.environment}${random_integer.deployment_id_suffix.result}mp"
  resource_group_name = azurerm_resource_group.rg.name
  sku_name            = "S1"
...
resource "azurerm_public_ip" "pip" {
  name                = "${var.class_name}${var.student_name}${var.environment}${random_integer.deployment_id_suffix.result}ip"
  resource_group_name = azurerm_resource_group.rg.name
  location            = azurerm_resource_group.rg.location
